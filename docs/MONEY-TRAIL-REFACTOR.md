...
**The lint wall.** Routes and components no longer import `cogsFor`/`costAsOf`/`exVat`/`vatOf` (the dashboard re-exports are gone), and an ESLint `no-restricted-imports` rule forbids them across `src/routes/**` **and** `src/components/**`, with a narrow `costAsOf` exception for `items.tsx`'s per-item detail. `dbAliases` stays required. A new surface that tries to recompute now fails `npm run lint`.
...
`vitest` + `@testing-library/react` + `@testing-library/jest-dom` + `@testing-library/dom` + `jsdom` were added as dev dependencies in `package.json`, plus `test` / `test:watch` scripts. No lockfile was committed (the repo tracks none); `npm install` reads them from `package.json`. `package-lock.json` was generated locally but left untracked.
...
- **A lockfile is now committed.** `package-lock.json` (lockfileVersion 3, 633 packages) was added so dependency versions resolve to a known-good set instead of fresh on every build. It was validated with a clean `npm ci` followed by `npm test` (53 passing) and `npm run build`. It was never in `.gitignore`; the repo simply had never generated or committed one, so this was an omission rather than a deliberate Lovable convention.
- **Node engine warnings on install are expected.** TanStack Start packages (via `@lovable.dev/vite-tanstack-config` to `@tanstack/react-start`) declare Node 22.12 or higher; the working machine runs Node 20.20.2. Install prints `EBADENGINE` warnings and everything works today (build, tests, dev). There is no `engines` field or `.npmrc` `engine-strict`, so these are advisory only and do not fail the install. Do not upgrade Node to chase them.
- **Do not run `npm audit fix`.** The reported vulnerabilities (about 10) are mostly in dev dependencies, and a forced fix pulls in breaking major upgrades. Leave them.
- **Lovable pipeline note.** The local `npm ci` / `npm run build` path works with the lockfile present. Whether Lovable's remote pipeline honors the committed lockfile or regenerates it could not be confirmed from the build environment. Either way is safe: a committed lockfile can only pin versions or be overwritten by the pipeline, never make the build worse than the previous no-lockfile state. Worth confirming on the next Lovable deploy that the lockfile is respected rather than silently replaced.
