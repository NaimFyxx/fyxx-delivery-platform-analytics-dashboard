-- Seed the versioned item_costs table from TGR_Platform_Item_Costs.csv.
-- item_name = the platform item name as it comes through the delivery reports
-- (matched case-insensitively + normalized at lookup time, see src/lib/costs.ts).
-- cost_exvat = cost_jod (ex-VAT cost basis, JOD). effective_from = 2025-01-01 so every
-- historical delivery order gets a cost. Idempotent: re-running inserts nothing new.
-- Future cost changes add NEW rows with a later effective_from (history preserved).

-- Optional provenance / approximation note (combos are component-built estimates).
...
FROM (
  VALUES
    ...
    -- Combos (ESTIMATES built from component costs)
